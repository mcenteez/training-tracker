...
import { planStatuses, type PlanStatus } from "@/modules/plans/db/schema";
...
export default async function PlansLibraryPage({
  ...
  const status = planStatuses.find(
    (value): value is PlanStatus => value === requestedStatus,
  );
  const plans = await withDatabase((database) =>
  ...
  return (
    <div className="space-y-6 py-7">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Multi-session schedules that organize workout templates by week and
            day.
          </p>
        </div>
        {canManage ? (
          <Button asChild>
            <Link href="/app/library/plans/new">
      ...
      {single(params.error) ? (
        ...
        >
          The plan changed before your request completed. Reload and try again.
        </p>
      ) : null}
      ...
      {plans.length === 0 ? (
        <div className="border border-dashed border-border px-6 py-14 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Adjust the filters or create the first reusable plan.
          </p>
        </div>
      ) : (
        <section className="grid gap-4 lg:grid-cols-2" aria-label="Plans">
          {plans.map((plan) => (
            <Card key={plan.id} className="rounded-md border-border/70">
              <CardHeader>
                <div className="flex items-start justify-between gap-3">
                  <CardTitle className="text-lg">
                    <Link
                      href={`/app/library/plans/${plan.id}`}
              ...
              <CardContent className="space-y-4">
                <p className="line-clamp-2 min-h-10 text-sm text-muted-foreground">
                  {plan.description || "No schedule description."}
                </p>
                <p className="text-xs text-muted-foreground">
                  {plan.scheduleSlotCount} scheduled sessions
                </p>
                {canManage ? (
